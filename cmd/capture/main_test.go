...
func TestCLI_TemplateCommand_GeneratesGroupedEnvExample(t *testing.T) {
	binary := buildBinary(t)

	tmpDir := t.TempDir()
	srcFile := filepath.Join(tmpDir, "app.js")
	goFile := filepath.Join(tmpDir, "main.go")
	outputFile := filepath.Join(tmpDir, ".env.example")

	if err := os.WriteFile(srcFile, []byte("console.log(process.env.API_SECRET)\nconsole.log(process.env.DATABASE_URL)\nconsole.log(process.env.PORT)\n"), 0644); err != nil {
		t.Fatalf("Failed to write JS source file: %v", err)
	}
	if err := os.WriteFile(goFile, []byte("package main\nimport \"os\"\nfunc main(){_ = os.Getenv(\"API_KEY\"); _ = os.Getenv(\"DATABASE_URL\")}\n"), 0644); err != nil {
		t.Fatalf("Failed to write Go source file: %v", err)
	}

	cmd := exec.Command(binary, "template", "--root", tmpDir, "--output", outputFile)
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		t.Fatalf("Expected template generation to succeed, got: %v\nStdout: %s\nStderr: %s", err, stdout.String(), stderr.String())
	}

	outputBytes, err := os.ReadFile(outputFile)
	if err != nil {
		t.Fatalf("Expected generated .env.example file, got read error: %v", err)
	}
	output := string(outputBytes)

	if !strings.Contains(output, "# Generated by capture on ") {
		t.Fatalf("Expected generated header, got:\n%s", output)
	}
	if !strings.Contains(output, "# General variables\nPORT=\n") {
		t.Fatalf("Expected general variable group for PORT, got:\n%s", output)
	}
	if !strings.Contains(output, "# API variables\nAPI_KEY=\nAPI_SECRET=\n") {
		t.Fatalf("Expected API group sorted alphabetically, got:\n%s", output)
	}
	if !strings.Contains(output, "# DATABASE variables\nDATABASE_URL=\n") {
		t.Fatalf("Expected DATABASE group in output, got:\n%s", output)
	}
	if strings.Count(output, "DATABASE_URL=") != 1 {
		t.Fatalf("Expected DATABASE_URL to appear once, got:\n%s", output)
	}

	apiKeyPos := strings.Index(output, "API_KEY=")
	apiSecretPos := strings.Index(output, "API_SECRET=")
	if apiKeyPos == -1 || apiSecretPos == -1 || apiKeyPos > apiSecretPos {
		t.Fatalf("Expected API group variables sorted alphabetically, got:\n%s", output)
	}

	if !strings.Contains(stdout.String(), "Generated "+outputFile+" with 4 variable(s)") {
		t.Fatalf("Expected generation summary in stdout, got: %s", stdout.String())
	}
}
